...
- ADK session services (`InMemory`/`VertexAi`/`Database`) have no Firestore TTL support, so a custom Firestore session layer implementing the `expireAt` TTL privacy design (PROJECT_HANDOFF.md §7) is required.
